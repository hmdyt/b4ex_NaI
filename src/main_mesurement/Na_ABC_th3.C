void Na_ABC_th3(){
	...
	//make hist
	TH3D *hist = new TH3D(
        "hist", "Na ;area1(A) ;area2(B) ;area4(C)",
        x_total_bin, x_bin_min, x_bin_max,
        y_total_bin, y_bin_min, y_bin_max,
        z_total_bin, z_bin_min, z_bin_max
        );
	...
	tree1->SetBranchAddress("area2", &file1_area2);
	tree1->SetBranchAddress("area4", &file1_area4);
	tree2->SetBranchAddress("area1", &file2_area1);
	tree2->SetBranchAddress("area2", &file2_area2);
	tree2->SetBranchAddress("area4", &file2_area4);
	tree3->SetBranchAddress("area1", &file3_area1);
	tree3->SetBranchAddress("area2", &file3_area2);
	tree3->SetBranchAddress("area4", &file3_area4);

	for(int i = 0; i < tree1->GetEntries(); i++){
		tree1->GetEntry(i);
		if (file1_area1 < 5000 && file1_area2 < 5000 && file1_area4 < 5000){ hist->Fill(file1_area1, file1_area2, file1_area4); }
	}

	for(int j = 0; j < tree2->GetEntries(); j++){
		tree2->GetEntry(j);
		if (file2_area1 < 5000 && file2_area2 < 5000 && file2_area4 < 5000){ hist->Fill(file2_area1, file2_area2, file2_area4); }
	}

	for(int j = 0; j < tree3->GetEntries(); j++){
		tree3->GetEntry(j);
		if (file3_area1 < 5000 && file3_area2 < 5000 && file3_area4 < 5000){ hist->Fill(file3_area1, file3_area2, file3_area4); }
	}
    ...
    hist->GetZaxis()->SetLimits(
		calc_kev(z_bin_min, cal["C"][0], cal["C"][1]),
		calc_kev(z_bin_max, cal["C"][0], cal["C"][1])
	);

    // set range
    hist->GetXaxis()->SetRangeUser(0, 600 * 1000);
    hist->GetYaxis()->SetRangeUser(0, 600 * 1000);
    hist->GetZaxis()->SetRangeUser(0, 600 * 1000);
